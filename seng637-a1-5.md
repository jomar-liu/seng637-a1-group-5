...
In addition, we realized that there is an interconnection between the ATM system functions such as withdrawal, deposit , transfers and balance inquiry. The defects we observed helped us understand how each of these functions relies on the other and if one has an error it affects the whole system performance. 
